...
#             host=os.getenv("REDIS_REMOTE", "localhost"),
#             port=6379,
...
# async def publish_to_redis(redis_conn, data, ticker):
#     """Publish data to Redis Pub/Sub."""
#     try:
#         data['event_id'] = f"gex3:{datetime.now().isoformat()}"
#         message_data = json.dumps(data)
#         await redis_conn.publish('gex3', message_data)
#         logger.info(f"Published data for {ticker} to Redis channel 'gex3'")
#     except Exception as e:
...
async def main():
    try:
        ...
        # redis_conn = None  # Start with None for redis_conn
